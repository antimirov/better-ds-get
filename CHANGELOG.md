...
- **Smart Foreground Service**: Live 3-second polling speedometer for active downloads, implemented via `@notifee/react-native`. Android 14 compatible.
...
### Changed
- **Search Timeout & Limits**: Reduced network timeout to 30 seconds with retry logic. Increased search result fetching from 100 to **300** items to capture more results from slower NAS search plugins (like 1337x) in a single poll.
- **Non-Blocking UI**: Eliminated all blocking `Alert.alert` dialogs in favor of native Android Toasts for success messages (adding tasks, uploading torrents).
